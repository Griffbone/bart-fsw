...
pca9563_err_t pca9563_set_pin_mode(pca9563_device_t *device, uint8_t pin, uint8_t mode) {
    ...
    // Send command byte to address config register 
    ret = pca9563_write_byte(device, PCA9563_REG_CONFIG);
    ...
    uint8_t config_val;
    ret = pca9563_read_byte(device, &config_val);
    if (ret != PCA9563_ERR_OK) {
        return ret;
    }
    ...
    ret = pca9563_write_byte(device, config_val);
    if (ret != PCA9563_ERR_OK) {
        return ret;
    }
...
pca9563_err_t pca9563_set_pin(pca9563_device_t *device, uint8_t pin, uint8_t value) {
    ...
    // Send command byte to address output port register 
    ret = pca9563_write_byte(device, PCA9563_REG_OUTPUT_PORT);
    if (ret != PCA9563_ERR_OK) {
        return ret;
    }
    ...
    uint8_t output_val;
    ret = pca9563_read_byte(device, &output_val);
    if (ret != PCA9563_ERR_OK) {
        return ret;
    }
    ...
    // Write new output port register value
    ret = pca9563_write_byte(device, output_val);
    if (ret != PCA9563_ERR_OK) {
        return ret;
    }
...
pca9563_err_t pca9563_write_byte(pca9563_device_t *device, uint8_t byte) {
    ...
    ret = HAL_I2C_Master_Transmit(device->hi2c, PCA9563_I2C_ADDR << 1, tx, 1, device->timeout);
